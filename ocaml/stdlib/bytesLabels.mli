...
val make : int -> char -> bytes @@ portable
(** [make n c] returns a new byte sequence of length [n], filled with
    the byte [c].
    @raise Invalid_argument if [n < 0] or [n > ]{!Sys.max_string_length}. *)

val init : int -> f:(int -> char) -> bytes @@ portable
...
val of_string : string -> bytes @@ portable
(** Return a new byte sequence that contains the same bytes as the
    given string. *)

val to_string : bytes -> string @@ portable
(** Return a new string that contains the same bytes as the given byte
    sequence. *)

val sub : bytes -> pos:int -> len:int -> bytes @@ portable
...
val sub_string : bytes -> pos:int -> len:int -> string @@ portable
(** Same as {!sub} but return a string instead of a byte sequence. *)

val extend : bytes -> left:int -> right:int -> bytes @@ portable
...
val fill : bytes -> pos:int -> len:int -> char -> unit @@ portable
...
val concat : sep:bytes -> bytes list -> bytes @@ portable
...
val cat : bytes -> bytes -> bytes @@ portable
...
val iter : f:(char -> unit) -> bytes -> unit @@ portable
(** [iter ~f s] applies function [f] in turn to all the bytes of [s].
    It is equivalent to [f (get s 0); f (get s 1); ...; f (get s
    (length s - 1)); ()]. *)

val iteri : f:(int -> char -> unit) -> bytes -> unit @@ portable
(** Same as {!iter}, but the function is applied to the index of
    the byte as first argument and the byte itself as second
    argument. *)

val map : f:(char -> char) -> bytes -> bytes @@ portable
...
val mapi : f:(int -> char -> char) -> bytes -> bytes @@ portable
(** [mapi ~f s] calls [f] with each character of [s] and its
    index (in increasing index order) and stores the resulting bytes
    in a new sequence that is returned as the result. *)

val fold_left : f:('acc -> char -> 'acc) -> init:'acc -> bytes -> 'acc @@ portable
(** [fold_left f x s] computes
    [f (... (f (f x (get s 0)) (get s 1)) ...) (get s (n-1))],
    where [n] is the length of [s].
    @since 4.13 *)

val fold_right : f:(char -> 'acc -> 'acc) -> bytes -> init:'acc -> 'acc @@ portable
(** [fold_right f s x] computes
    [f (get s 0) (f (get s 1) ( ... (f (get s (n-1)) x) ...))],
    where [n] is the length of [s].
    @since 4.13 *)

val for_all : f:(char -> bool) -> bytes -> bool @@ portable
(** [for_all p s] checks if all characters in [s] satisfy the predicate [p].
    @since 4.13 *)

val exists : f:(char -> bool) -> bytes -> bool @@ portable
(** [exists p s] checks if at least one character of [s] satisfies the predicate
    [p].
    @since 4.13 *)

val trim : bytes -> bytes @@ portable
(** Return a copy of the argument, without leading and trailing
    whitespace. The bytes regarded as whitespace are the ASCII
    characters [' '], ['\012'], ['\n'], ['\r'], and ['\t']. *)

val escaped : bytes -> bytes @@ portable
...
val index_from_opt: bytes -> int -> char -> int option @@ portable
...
val rindex_from : bytes -> int -> char -> int @@ portable
...
val rindex_from_opt: bytes -> int -> char -> int option @@ portable
...
val contains : bytes -> char -> bool @@ portable
(** [contains s c] tests if byte [c] appears in [s]. *)

val contains_from : bytes -> int -> char -> bool @@ portable
(** [contains_from s start c] tests if byte [c] appears in [s] after
    position [start].  [contains s c] is equivalent to [contains_from
    s 0 c].
    @raise Invalid_argument if [start] is not a valid position in [s]. *)

val rcontains_from : bytes -> int -> char -> bool @@ portable
(** [rcontains_from s stop c] tests if byte [c] appears in [s] before
    position [stop+1].
    @raise Invalid_argument if [stop < 0] or [stop+1] is not a valid
    position in [s]. *)

val uppercase_ascii : bytes -> bytes @@ portable
(** Return a copy of the argument, with all lowercase letters
   translated to uppercase, using the US-ASCII character set.
   @since 4.03 (4.05 in BytesLabels) *)

val lowercase_ascii : bytes -> bytes @@ portable
(** Return a copy of the argument, with all uppercase letters
   translated to lowercase, using the US-ASCII character set.
   @since 4.03 (4.05 in BytesLabels) *)

val capitalize_ascii : bytes -> bytes @@ portable
(** Return a copy of the argument, with the first character set to uppercase,
   using the US-ASCII character set.
   @since 4.03 (4.05 in BytesLabels) *)

val uncapitalize_ascii : bytes -> bytes @@ portable
...
val starts_with :
  prefix (* comment thwarts tools/sync_stdlib_docs *) :bytes -> bytes -> bool @@ portable
...
val ends_with :
  suffix (* comment thwarts tools/sync_stdlib_docs *) :bytes -> bytes -> bool @@ portable
...
val unsafe_to_string : bytes -> string @@ portable
...
val unsafe_of_string : string -> bytes @@ portable
...
val split_on_char: sep:char -> bytes -> bytes list @@ portable
...
val set_utf_8_uchar : t -> int -> Uchar.t -> int @@ portable
...
val set_utf_16be_uchar : t -> int -> Uchar.t -> int @@ portable
...
val set_utf_16le_uchar : t -> int -> Uchar.t -> int @@ portable
...
external unsafe_blit :
  src:bytes -> src_pos:int -> dst:bytes -> dst_pos:int -> len:int ->
    unit @@ portable = "caml_blit_bytes" [@@noalloc]
external unsafe_blit_string :
  src:string -> src_pos:int -> dst:bytes -> dst_pos:int -> len:int -> unit @@ portable
  = "caml_blit_string" [@@noalloc]
external unsafe_fill :
  bytes -> pos:int -> len:int -> char -> unit @@ portable = "caml_fill_bytes" [@@noalloc]

val unsafe_escape : bytes -> bytes @@ portable
