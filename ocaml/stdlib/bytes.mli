...
val init : int -> (int -> char) -> bytes @@ portable
...
val sub : bytes -> int -> int -> bytes @@ portable
...
val sub_string : bytes -> int -> int -> string @@ portable
(** Same as {!sub} but return a string instead of a byte sequence. *)

val extend : bytes -> int -> int -> bytes @@ portable
...
val fill : bytes -> int -> int -> char -> unit @@ portable
...
val concat : bytes -> bytes list -> bytes @@ portable
...
val iter : (char -> unit) -> bytes -> unit @@ portable
(** [iter f s] applies function [f] in turn to all the bytes of [s].
    It is equivalent to [f (get s 0); f (get s 1); ...; f (get s
    (length s - 1)); ()]. *)

val iteri : (int -> char -> unit) -> bytes -> unit @@ portable
(** Same as {!iter}, but the function is applied to the index of
    the byte as first argument and the byte itself as second
    argument. *)

val map : (char -> char) -> bytes -> bytes @@ portable
...
val mapi : (int -> char -> char) -> bytes -> bytes @@ portable
(** [mapi f s] calls [f] with each character of [s] and its
    index (in increasing index order) and stores the resulting bytes
    in a new sequence that is returned as the result. *)

val fold_left : ('acc -> char -> 'acc) -> 'acc -> bytes -> 'acc @@ portable
(** [fold_left f x s] computes
    [f (... (f (f x (get s 0)) (get s 1)) ...) (get s (n-1))],
    where [n] is the length of [s].
    @since 4.13 *)

val fold_right : (char -> 'acc -> 'acc) -> bytes -> 'acc -> 'acc @@ portable
(** [fold_right f s x] computes
    [f (get s 0) (f (get s 1) ( ... (f (get s (n-1)) x) ...))],
    where [n] is the length of [s].
    @since 4.13 *)

val for_all : (char -> bool) -> bytes -> bool @@ portable
(** [for_all p s] checks if all characters in [s] satisfy the predicate [p].
    @since 4.13 *)

val exists : (char -> bool) -> bytes -> bool @@ portable
...
val split_on_char: char -> bytes -> bytes list @@ portable
...
external unsafe_fill :
  bytes -> int -> int -> char -> unit @@ portable = "caml_fill_bytes" [@@noalloc]
